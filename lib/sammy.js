;(function($) {
  ...
  Sammy.LocationProxy = function(app) {
    this.app = app;
    this.location = "";
  };
  
  Sammy.LocationProxy.prototype = {
  ...
  Sammy.HashLocationProxy.prototype = {
    ...
    _startPolling: function(every) {
      ...
      if (!Sammy.HashLocationProxy._interval) {
        if (!every) { every = 50; }
        Sammy.HashLocationProxy._interval = setInterval(function() {
          if (Sammy.HashLocationProxy._last_hash && 
              window.location.hash != Sammy.HashLocationProxy._last_hash) {
            $(window).trigger('hashchange')
          } 
          Sammy.HashLocationProxy._last_hash = window.location.hash;
        }, every);
