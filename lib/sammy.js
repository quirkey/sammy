...
(function($) {
    ...
    then: function(callback) {
      if (_isFunction(callback)) { 
        ...
        if (this.waiting) {
          ...
          this.wait();
          var returned = callback.apply(this, [this.content, this.previous_content]);
          if (returned !== false) {
  ...
  Sammy.EventContext.prototype = $.extend({}, Sammy.Object.prototype, {
    ...
    engineFor: function(path) {
      ...
      if (_isFunction(path)) { return path; }
      // lookup engine name by path extension
      if ((engine = path.match(/\.([^\.]+)$/))) { engine = engine[1]; }
      // set the engine to the default template engine if no match is found
      if ((!engine || !_isFunction(context[engine])) && context.app.template_engine) {
