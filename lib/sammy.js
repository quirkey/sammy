  ...
  Sammy.Application = Sammy.Object.extend({
    ...
    run: function() {
      ...
      $('form').bind('submit', function() {
        return app._checkFormSubmission();
      });
    ...
    runRoute: function(verb, path) {
      this.log('runRoute', verb, path);
      var route = this.lookupRoute(verb, path)
      if (route) {
        // set event context
        var context = new Sammy.EventContext(this, verb, path, {}); // stubbed out params
        route.callback.apply(context);
    ...
    _checkURL: function() {
      ...
      this.log('_checkURL', this, location);
      // compare to see if hash has changed
      try {
        this.log('now:', location.hash, 'last:', this.last_location.hash);
