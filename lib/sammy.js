  ...
  Sammy.FormParametersParser = {
    parseForm: function(form) {
      ...
      $.each(form.serializeArray(), function(i, field) {
        $.extend(true, params, context._insertFieldIntoParameters(field.value, field.name, params));
      });
      
      return params;
    },
    
    _insertFieldIntoParameters: function(field_value, field_name, params) {
      var context = this;
      var match, name, rest;
      ...
      if (field_name.match(/^[^\[]+$/)) {
        ...
        if (rest) {
          ...
          if (params[name]) {
            if (!$.isArray(params[name])) { throw('400 Bad Request'); }
            if(params[name][params[name].length - 1][match[1]]) {
              params[name].push(context._insertFieldIntoParameters(field_value, match[1] + match[2], {}));  
            } else {
              $.extend(true, params[name][params[name].length - 1], context._insertFieldIntoParameters(field_value, match[1] + match[2], {}));  
            }            
          } else {
            params[name] = [context._insertFieldIntoParameters(field_value, match[1] + match[2], {})];
          }                    
        } else {
          if (params[name]) {
            if (!$.isArray(params[name])) { throw('400 Bad Request'); }
        ...
        if (params[name]) {
          if ($.isArray(params[name])) { throw('400 Bad Request'); }
          $.extend(true, params[name], context._insertFieldIntoParameters(field_value, next + rest, params[name]));
        } else {
          params[name] = context._insertFieldIntoParameters(field_value, next + rest, {});
        }
      }
      return params;
    }
