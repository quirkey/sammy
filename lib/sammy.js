...
(function($) {
  ...
  $.srender = function(name, template, data, target){
    $.srender.cache = {};
    ...
    if ($.srender.cache[name]) {
      ...
      if (typeof template == 'undefined') {
        // we just want to fetch the cached template function
        return fn;
      }
    } else {
...
(function($) {
  ...
  Sammy.EventContext = Sammy.Object.extend({
    ...
    template: function(template, data) {
      return $.srender(this.app.namespace + ':' + template, template, $.extend({}, data, this));
    },
    ...
    _renderTemplate: function(selector, content, options) {
      var $selector = $(selector).html($.srender(content, content, options));
