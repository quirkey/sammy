...
(function($, window) {
  ...
  _extend(Sammy.Object.prototype, {
    ...
    has: function(key) {
      return this[key] && _trim(this[key].toString()) != '';
    },
  ...
  Sammy.Adapter.jQuery = {
    ...
    addDomListener: function(selector, name, callback) {
      return $(selector).bind(name, callback);
    },

    removeDomListener: function(selector, name) {
      return $(selector).unbind(name);
    },

    triggerDomEvent: function(selector, name, data) {
      return $(selector).trigger(name, data);
    },

    listenForFormSubmission: function(selector, checkCallback) {
      return $(selector).bind('submit', function(e) {
        var returned = checkCallback($(e.target).closest('form')[0]);
        return (returned === false) ? e.preventDefault() : false;
      });
    },

    unlistenForFormSubmission: function(selector) {
      return $(selector).unbind('submit');
    },
  ...
  Sammy.Application.prototype = _extend({}, Sammy.Object.prototype, {
    ...
    runRoute: function(verb, path, params, target, all) {
      ...
      if (!this.isRunning()) {
        this.log('App is not running. routes and events are disabled.');
        this.log('Tried running ', verb, path);
        return;
      }
