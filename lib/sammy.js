...
;(function($) {
  ...
  Sammy.Application.prototype = $.extend({}, Sammy.Object.prototype, {
    ...
    before: function(options, callback) {
      ...
      return this.befores.push([options, callback]);
    },
    ...
    runRoute: function(verb, path, params) {
      ...
      var context, befores, before, before_value, returned,
      ...
      if (route) {
        ...
        before_value = true; 
        befores = this.befores.slice(0); // ensure array
        while (befores.length > 0) {
          before = befores.shift();
          // check the options
          if (this.routeMatchesOptions(before[0], route)) {
            if (before[1].apply(context) === false) return false;
          }
        }
    ...
    routeMatchesOptions: function(route, options, positive) {
      ...
      if (options.only) {
        return this.routeMatchesOptions(route, options.only, true);
      } else if (options.except) {
        return this.routeMatchesOptions(route, options.except, false);  
      }
      var matched = positive;
      if (options.path) {
        if (typeof options.path == 'regexp') {
          matched = route.path.test(options.path);
        } else {
          matched = (options.path.toString() == route.path);
        }
      }
      if (options.verb) {
        matched = matched && (options.verb == route.verb);
      }
