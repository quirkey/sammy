...
(function($) {
  ...
  Sammy.Application = Sammy.Object.extend({
    ...
    route: function(verb, path, callback) {
      ...
      if (typeof this.routes[verb] == 'undefined')  {
        ...
        $.each(this.routes[verb], function(i, route)  {
          console.log('adding route each', route, i);
    ...
    bind: function(name, data, callback) {
      ...
      return $(this.element_selector).bind(this.eventNamespace() + ':' + name, data, function() {
        // pull off the context from the arguments to the callback
        return callback.apply(arguments[1], [arguments[0], arguments[2]]);
      });
    },
    
    trigger: function(name, data, context) {
      if (typeof context == 'undefined') context = this;
      return $(this.element_selector).triggerHandler(this.eventNamespace() + ':' + name, [context, data]);
    },
    
    run: function() {
      var app = this;
      // set data for app
      $(this.element_selector).data(this.data_store_name, this);
      // set last location
    ...
    runRoute: function(verb, path, params) {
      ...
      if (route) {
        this.log('runRoute', 'route found', route);
