  ...
  $.extend(Sammy.RenderContext.prototype, {
    ...
    load: function(path, params, callback) {
      ...
      return this.then(function() {
        ...
        // var cached = this.event_context.app.remoteCache(path);
        // if (cached) {
        // 
        // } else {
        // 
        // }
        Sammy.log('load', path, params, callback);
        this.wait();
        if (callback) { this.then(callback); }
        $.get(path, params, function(data) {
          Sammy.log('loaded', data, context);
    ...
    render: function(path, data, callback) {
      return this.load(path).interpolate(data, path).then(callback);
    },
    ...
    renderEach: function(path, name, data, callback) {
      ...
      return this.load(path).each(data, function(i, value) {
        var idata = {};
        name ? (idata[name] = value) : (idata = value);
        return this.interpolate(idata, path);
      });
    },
    
    then: function(callback) {
      Sammy.log('then', 'waiting', this.waiting, 'callback', callback, this.callbacks);
      if ($.isFunction(callback)) { 
        if (this.waiting) {
          this.callbacks.push(callback);
        } else {
          this.wait();
          var returned = callback.apply(this, [this.content]);
          if (returned !== false) {
          ...
          Sammy.log('returned', returned, this.content);
        }
      }
      return this;
    },
    ...
    interpolate: function(data, engine) {
      ...
      return this.then(function(content) {
        Sammy.log('interpolate', content, data, engine, context.event_context);
        return context.engineFor(engine).apply(context.event_context, [content, data]);
      });
    },
    
    engineFor: function(path) {
      var context = this.event_context, 
          engine;
      ...
      if ((!engine || !$.isFunction(context[engine])) && context.app.template_engine) {
        engine = context.app.template_engine;
      }
