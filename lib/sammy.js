...
(function($) {
  ...
  Sammy.EventContext.prototype = $.extend({}, Sammy.Object.prototype, {
    ...
    //    });
    // 
    partial: function(path, data, callback, partial_paths) {
      var file_data, 
          wrapped_callback,
          engine,
          data_array,
      ...
      wrapped_callback = function(response, partials) {
        var new_content = response,
            all_content =  "";
        $.each(data_array, function(i, idata) {
          if ($.isFunction(engine)) {
            new_content = engine.apply(context, [response, idata, partials]);
          } 
          // collect the content
      ...
      render_template = function(partials) {
        if (context.app.cache_partials && context.cache(cache_key)) {
          // try to load the template from the cache
          wrapped_callback.apply(context, [context.cache(cache_key), partials]);
        } else {
          // the template wasnt cached, we need to fetch it
          $.get(path, function(response) {
            if (context.app.cache_partials) { context.cache(cache_key, response); }
            wrapped_callback.apply(context, [response, partials]);
          });
        }        
      };
      gather_partials = function(partials, partial_paths, partial_names) {
        if(partial_paths && partial_names.length > 0) {
          var name = partial_names.shift();
          var partial_path = partial_paths[name];
          
          if(context.app.cache_partials && context.cache('partial:' + partial_path)) {
            partials[name] = context.cache('partial:' + partial_path);
            gather_partials(partials, partial_paths, partial_names);            
          } else {
            $.get(partial_path, function(response) {
              if(context.app.cache_partials) { context.cache('partial:' + partial_path, response); }
              partials[name] = response;
              gather_partials(partials, partial_paths, partial_names);
            });            
          }
        } else {
          render_template(partials);
        }
      };
      for(var name in partial_paths) { partial_names.push(name); }
      gather_partials({}, partial_paths, partial_names);
    },
