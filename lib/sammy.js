;(function($) {
  ...
  Sammy.Application = function(app_function) {
    ...
    this.form_parameters_parser = Sammy.FormParametersParser;
  ...
  Sammy.Application.prototype = $.extend({}, Sammy.Object.prototype, {
    ...
    _checkFormSubmission: function(form) {
      ...
      params = this.form_parameters_parser.parseForm($form);
  ...
  Sammy.FormParametersParser = {
    parseForm: function(form) {
      var params = {'$form': form};
      var context = this;
      $.each(form.serializeArray(), function(i, field) {
        $.extend(true, params, context._insertFieldIntoParameters(field, params));
      });
      
      return params;
    },
    
    _insertFieldIntoParameters: function(field, params) {
      var context = this;
      if (field.name.match(/\[[^\]]+\]/)) {
        ...
        if (params[name]) {
          start = params[name];
          $.extend(true, params[name], context._normalizeParameters(unescape(field.value), rest, start));
        } else {
          start = {};
          params[name] = context._normalizeParameters(unescape(field.value), rest, start);
        }
        console.log(name);
        console.log(params[name]);
      } else if (field.name.match(/\[\]/)) {
        $.extend(true, params, context._parseArray(field.value, field.name, params));
      } else {
        params[field.name] = unescape(field.value);
      }
      return params;
    },
    
    _parseArray: function(value, param, params) {
      var name = param.match(/(.*)\[\]/)[1];
      
      if ($.isArray(params[name])) {
        params[name].push(unescape(value));
      } else {
    ...
    _normalizeParameters: function(value, param, params) {
      ...
      if (rest) {
        if (rest == '[]') {
          result = this._parseArray(value, name + '[]', params);
        } else {
          if (!params[name]) {
            params[name] = {};
          }
          result[name] = $.extend(true, params[name], this._normalizeParameters(value, rest, params[name]));
        }
      } else {
        result[name] = value;
      }
