...
(function($) {
  ...
  Sammy.Application = Sammy.Object.extend({
    ...
    lookupRoute: function(verb, path) {
      var routed = false, parts = path.match(/^(.*#.*)\?/);
      if (parts) {
          // Ignore any query string in the path.
          path = parts[1];
      }
    ...
    // Pull out a query string from the path fragment.  Returns a pair of the
    // given path with the query string removed, and an object representing
