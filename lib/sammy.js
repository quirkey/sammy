;(function($) {
  ...
  Sammy.HashLocationProxy.prototype = {
    ...
    setLocation: function(new_location) {
      Sammy.log('setLocation', new_location);
    ...
    _startPolling: function(every) {
      ...
      if (!Sammy.HashLocationProxy._interval) {
        Sammy.log('startPolling');
        if (!every) every = 10;
        var hashCheck = function() {
          current_location = proxy.getLocation();
          Sammy.log('getLocation', current_location);
          if (!Sammy.HashLocationProxy._last_hash || current_location != Sammy.HashLocationProxy._last_hash) {
            Sammy.log('triggering hashchange');
            $(window).triggerHandler('hashchange');
          } 
          Sammy.HashLocationProxy._last_hash = current_location;
        }
        hashCheck();
