...
(function($) {
  ...
  $.extend(Sammy.RenderContext.prototype, {
        ...
        if (typeof location === 'string') { 
          ...
          $.ajax($.extend({
            ...
            success: function(data) {
              if (should_cache) {
                this.event_context.app.templateCache(location, data);
              }
    ...
    render: function(location, data, callback) {
      if (_isFunction(location) && !data) {
        ...
        return this.load(location).interpolate(data, engine).then(callback);
      }
    },
  ...
  Sammy.EventContext.prototype = $.extend({}, Sammy.Object.prototype, {
    ...
    engineFor: function(engine) {
      ...
      if ((engine = engine.match(/\.([^\.]+)$/))) { engine = engine[1]; }
