(function($) {
  ...
  $.extend(Sammy.Store.prototype, {
    ...
    set: function(key, value, callback) {
      ...
      this.storage.set(key, string_value, function() {
        store.$element.trigger('set-' + store.name, {key: key, value: value});
        store.$element.trigger('set-' + store.name + '-' + key, {key: key, value: value});
  ...
  $.extend(Sammy.Store.Memory.prototype, {
    ...
    clear: function(key, callback) {
      delete Sammy.Store.Memory.store[this.namespace][key];
  ...
  Sammy.Store.LocalStorage = function(name, element) {
    this.name = name;
    this.element = element;
    this.key_prefix = ['store', this.element, this.name].join('.');
  ...
  $.extend(Sammy.Store.LocalStorage.prototype, {
    ...
    get: function(key, callback) {
      var value = window.localStorage.getItem(this._key(key));
