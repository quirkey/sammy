(function($) {
  ...
  $.extend(Sammy.Store.prototype, {
    ...
    set: function(key, value, callback) {
      ...
      this.storage.set(key, string_value, function() {
        if (key != store.meta_key) {
          store._addKey(key);
          store.$element.trigger('set-' + this.name, {key: key, value: value});
          store.$element.trigger('set-' + this.name + '-' + key, {key: key, value: value});
        }
    ...
    keys: function(callback) {
      var store = this;
      this.get(this.meta_key, function(keys) {
        _invoke(callback, store, keys || []);
      });
    },
  ...
  $.extend(Sammy.Store.Memory.prototype, {
    ...
    exists: function(key, callback) {
      return _invoke(callback, false, typeof this.store[key] != "undefined");
    },
    set: function(key, value, callback) {
