(function(window) {
  ...
  function getStringContent(object, content) {
    if (typeof content === 'undefined') {
      return '';
    } else if (Sammy.isFunction(content)) {
  ...
  Sammy.extend(Sammy.FormBuilder.prototype, {

    // creates the open form tag with the object attributes
    open: function(attributes) {
      return simple_element('form', Sammy.extend({'method': 'post', 'action': '#/' + this.name + 's'}, attributes), false);
    },
    ...
    label: function(keypath, content, attributes) {
      ...
      return simple_element('label', Sammy.extend(attrs, attributes), content);
    },

    // creates a hidden input for `keypath` with an optional `attributes` object
    hidden: function(keypath, attributes) {
      attributes = Sammy.extend({type: 'hidden'}, this._attributesForKeyPath(keypath), attributes);
    ...
    text: function(keypath, attributes) {
      attributes = Sammy.extend({type: 'text'}, this._attributesForKeyPath(keypath), attributes);
      return simple_element('input', attributes);
    },
    ...
    textarea: function(keypath, attributes) {
      var current;
      attributes = Sammy.extend(this._attributesForKeyPath(keypath), attributes);
    ...
    password: function(keypath, attributes) {
      return this.text(keypath, Sammy.extend({type: 'password'}, attributes));
    },
    ...
    select: function(keypath, options, attributes) {
      var option_html = "", selected, i = 0;
      attributes = Sammy.extend(this._attributesForKeyPath(keypath), attributes);
      selected = attributes.value;
      delete attributes['value'];
      var value, text, option_attrs, option;
      for (; i < options.length; i++) {
        if (Sammy.isArray(option)) {
    ...
    radio: function(keypath, value, attributes) {
      ...
      return simple_element('input', Sammy.extend({type:'radio'}, attributes));
    },
    ...
    checkbox: function(keypath, value, attributes) {
      ...
      content += this.radio(keypath, value, Sammy.extend({type: 'checkbox'}, attributes));
      return content;
    },

    // creates a submit input for `keypath` with an optional `attributes` object
    submit: function(attributes) {
      return simple_element('input', Sammy.extend({'type': 'submit'}, attributes));
    },

    _attributesForKeyPath: function(keypath) {
      var builder    = this,
          keys       = Sammy.isArray(keypath) ? keypath : keypath.split(/\./),
          name       = builder.name,
