(function($) {
  ...
  function parseNestedParam(params, field_name, field_value) {
    ...
    if (field_name.match(/^[^\[]+$/)) {
      // basic value
      field_value = field_value.trim(); // trim the value
      //if (field_value !== '')
...
//
// thanks: http://glyphobet.net/configurable-serialize/#demo
//
(function ($) {
		...
		.filter(function(){
			return this.name && !this.disabled &&
				((this.checked || (options.emptyCheckboxes && options.emptyValues && this.type == 'checkbox') ) ||
					...
					/color|date|datetime|email|file|hidden|month|number|password|range|search|tel|text|time|url|week/i.test(this.type));
		})
		.map(function(i, elem){
