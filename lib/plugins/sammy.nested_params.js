(function(window) {
  ...
  function parseNestedParam(params, field_name, field_value) {
    ...
    if (field_name.match(/^[^\[]+$/)) {
      ...
      if(params[name] && !Sammy.isArray(params[name])) { throw('400 Bad Request'); }
      ...
      if (rest) {
        ...
        if (params[name]) {
          if(params[name][params[name].length - 1][match[1]]) {
            ...
            Sammy.extend(params[name][params[name].length - 1], parseNestedParam({}, match[1] + match[2], field_value));
          }
        } else {
      ...
      if (params[name]) {
        Sammy.extend(params[name], parseNestedParam(params[name], rest, field_value));
      } else {
